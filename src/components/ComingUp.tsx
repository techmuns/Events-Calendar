...
import { BellIcon, MicIcon, StarIcon, XIcon } from "./icons";
...
// On-open reminder strip: the soonest upcoming events (watchlist first), so the
// client immediately sees what's about to happen, plus a handy row of
// just-announced earnings calls. Not a "since your last visit" diff.
    ...
    >
      ...
      <div
        style={{
          ...
          gap: 12,
          padding: "11px 14px",
          borderBottom: reminders.length || calls.length ? `1px solid ${tokens.border}` : "none",
      ...
      {calls.length > 0 && (
        <div style={{ borderTop: reminders.length ? `1px solid ${tokens.border}` : "none", padding: "9px 14px 11px", background: reminders.length ? "var(--detail-header-bg)" : "transparent" }}>
